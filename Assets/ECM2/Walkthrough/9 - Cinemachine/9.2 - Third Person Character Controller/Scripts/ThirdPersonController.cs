    ...
    public class ThirdPersonController : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if(!GameManager.Instance.skyDive)
            {
                ...
                {
                    GameManager.Instance.PlayAnimation("Walk");
                }
                else
                {
                    GameManager.Instance.PlayAnimation("Idle");
                }
                ...
                for (int i = 0; i < Input.touchCount; i++)
                {
                    ...
                    if (!EventSystem.current.IsPointerOverGameObject(touch.fingerId))
                    {
                        if (touch.phase == TouchPhase.Moved)
                        {
                            ...
                            float yaw = Mathf.Lerp(0, lookInput.x * lookSensitivity.x * 4, Time.deltaTime);
                            float pitch = Mathf.Lerp(0, lookInput.y * lookSensitivity.y * 4, Time.deltaTime);

                            AddControlYawInput(yaw);
                            AddControlPitchInput(pitch, minPitch, maxPitch);
                        }
                    }
                }
            }
